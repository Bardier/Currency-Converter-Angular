...
export class BodyComponent implements OnInit {
  ...
  onFirstIn(inputValue: string, firstSel: string, secondSel: string) {
    ...
    this.secondInput = isNaN(parseFloat(res)) ? '' : parseFloat(res).toFixed(2);
  }

  onSecondIn(inputValue: string, firstSel: string, secondSel: string) {
    this.secondInput = inputValue.replace(/[^0-9\.]/g, '');
    if (!inputValue) {
      return;
    }
    ...
    this.firstInput = isNaN(parseFloat(res)) ? '' : parseFloat(res).toFixed(2);
  }
  ngOnInit(): void {}
}
